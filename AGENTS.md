...
To add another assistant (e.g. Cursor, Gemini CLI), add a symlink pointing at `.agents/skills/`. Do not duplicate the content. On Windows the `.claude/skills` symlink resolves only with git symlinks enabled (`git config core.symlinks true`, plus Developer Mode or admin); otherwise use the canonical `.agents/skills/` path directly.
