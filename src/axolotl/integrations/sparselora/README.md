...
- **Architecture: model-agnostic.** SwiGLU-MLP + standard-attention transformers are supported and auto-detected at apply time (`arch_wiring.register_arch_wiring`): MLP sparsity is universal (the SwiGLU forward is identical across families), and a generic `SparseAttention` handles per-arch differences it introspects — projection bias (Qwen2), `q_norm`/`k_norm` on the head dim (Qwen3), and `sliding_window` (Qwen2/Qwen3/Mistral). Verified on Llama, Qwen2, Qwen3, Mistral. Attention-logit softcapping (Gemma2/Gemma3) is **rejected** at validation because the generic path does not apply it; extend any other custom attention via `register_sparse_module`. Adding a new architecture is documented in the [`sparselora-add-model`](../../../../.agents/skills/sparselora-add-model/SKILL.md) agent skill.
