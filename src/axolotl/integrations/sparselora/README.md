...
Contextual-sparsity acceleration for LoRA fine-tuning, integrated as an Axolotl plugin. Wraps [z-lab/sparselora](https://github.com/z-lab/sparselora) (ICML 2025, MIT; vendored under `_vendor/`) and adds **self-calibration** (the per-layer sparsity schedule and the SVD predictor factors are derived from *your* base model and *your* dataset, rather than z-lab's Llama-2/3-only published presets) and **model-agnostic wiring** (Llama, Qwen2, Qwen3, Mistral, and any other SwiGLU-MLP / standard-attention transformer, auto-detected at apply time rather than the upstream Llama-only hardcoding).
...
- **Architecture: model-agnostic.** SiLU-gated SwiGLU-MLP + standard-attention transformers are supported and auto-detected at apply time (`arch_wiring.register_arch_wiring`): MLP sparsity is universal (the SwiGLU forward is identical across families), and a generic `SparseAttention` handles per-arch differences it introspects — projection bias (Qwen2), `q_norm`/`k_norm` on the head dim (Qwen3), `sliding_window` (Qwen2/Qwen3/Mistral), decoupled `head_dim` (Qwen3: q_out ≠ hidden, via a fixed GQA predictor), and Gemma2 attention-logit softcapping (forwarded to the attention interface). Verified on Llama, Qwen2, Qwen3 (incl. real Qwen3-0.6B), Mistral, StableLM, Cohere. **Rejected:** non-SiLU gated MLPs (Gemma2/Gemma3 use `gelu_pytorch_tanh`, which the SiLU-hardcoded FFN predictor + sparse MLP kernel can't reproduce) and fused-projection models (Phi3's `qkv_proj`/`gate_up_proj`). Extend custom wiring via `register_sparse_module`. Adding a new architecture is documented in the [`sparselora-add-model`](../../../../.agents/skills/sparselora-add-model/SKILL.md) agent skill.
