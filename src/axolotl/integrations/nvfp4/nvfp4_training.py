...
def _sm120f_recipe_codegen_verified() -> bool:
    ...
    the cvt ops are the only numerics and bit-exact is achievable). Default-safe
    (False on error); ``AXOLOTL_NVFP4_SKIP_SM120F_PROBE=1`` force-disables.
    """
    ...
    try:
        ...
        # Gate must not open without MSLK (the dispatcher falls back to it).
...
def load_nvfp4_packed(model: nn.Module, model_dir) -> int:
    ...
    if not os.path.isfile(path):
        return 0
    # Trusted locally-produced sidecar (same trust as the HF checkpoint the
    # Trainer itself loads); weights_only can't rebuild the NVFP4Tensor subclass.
