...
class NVFP4Args(BaseModel):
    ...
    # Flat compatibility bridge — read-only views the relocated swap/kernel code reads.
    @property
    ...
    def skip_last_n_blocks(self) -> int:
        # The "paper" preset's tail (round(0.13*L)) needs the block count, resolved in swap.py.
        if self.keep_hp_blocks == "paper":
            return 0
