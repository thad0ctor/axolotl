...
class AxolotlTrainingMixins:
    ...
    image_tiling: bool | None = field(
        default=False,
        metadata={"help": "Enable image tiling for multimodal CPT/SFT"},
    )

    image_tiling_tile_size: int | tuple[int, int] | None = field(
        default=None,
        metadata={"help": "Processor canvas for each tiled image crop"},
    )

    image_tiling_grid: tuple[int, int] | None = field(
        default=None,
        metadata={"help": "Default image tiling grid as (columns, rows)"},
    )

    image_tiling_overlap: float | None = field(
        default=0.1,
        metadata={"help": "Tile overlap fraction per cell edge"},
    )

    image_tiling_min_area: int | None = field(
        default=0,
        metadata={"help": "Only tile images with area at least this value"},
    )

    image_tiling_overview_size: int | tuple[int, int] | None = field(
        default=None,
        metadata={"help": "Optional overview image canvas prepended before tiles"},
    )

    image_tiling_overview_buckets: list[tuple[int, int]] | None = field(
        default=None,
        metadata={"help": "Optional overview image canvas buckets"},
    )

    image_tiling_no_upscale: bool | None = field(
        default=True,
        metadata={"help": "Do not upscale page overviews or tiles"},
    )

    image_tiling_pad_color: int | tuple[int, int, int] | str | None = field(
        default=None,
        metadata={"help": "Padding color for tiled image canvases"},
    )

    image_tiling_reading_order: str | None = field(
        default="rtl",
        metadata={"help": "Column traversal order for tiled page rows"},
    )

    image_tiling_cache_path: str | None = field(
        default=None,
        metadata={"help": "SSD path for cached processor-ready image tiles"},
    )

    image_tiling_cache_hash_images: bool | None = field(
        default=False,
        metadata={"help": "Include full source image sha256 in tile cache keys"},
    )

    image_tiling_shape_buckets: list[dict] | str | None = field(
        default=None,
        metadata={"help": "Aspect-ratio tiling policies or 'ocr_pages' preset"},
    )
