"""Experimental FP8 lm_head projection.

Default-off helper for testing FP8 as a middle ground between bf16 and the
accuracy-blocked FP4 lm_head. It uses PyTorch's scaled FP8 matmul and keeps the
weight prepacked so benchmarks do not pay a per-step weight quantization cost.
"""
...
def _quantize_e4m3(x: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
    q = (x.float() / scale).clamp(-_E4M3_MAX, _E4M3_MAX).to(torch.float8_e4m3fn)
    return q
...
def prepack_lm_head_weight_fp8(
    ...
    *,
    ...
    ``rowwise`` means one scale per vocab row, represented as per-column scales
    for the transposed ``[hidden, vocab]`` operand used by ``torch._scaled_mm``.
    """
    if weight.ndim != 2:
...
def fp8_lm_head(
    ...
    *,
    ...
    ``hidden`` may be ``[..., hidden]`` and is dynamically quantized per call.
    The output is returned in ``hidden.dtype`` unless ``out_dtype`` is provided.
    """
