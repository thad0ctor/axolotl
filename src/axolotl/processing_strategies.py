...
class ProcessingStrategy:
    ...
    def __call__(self, examples: list[dict]) -> list[dict]:
        ...
        def convert_messages_to_multimedia_messages(
            messages: list[dict], deserialize_json_content: bool = False
        ) -> list[dict]:
        ...
        for example in examples:
            ...
            # deserialize_json_content handles multi-image mixed content for
            # models that support it (see supports_multi_images).
            ...
            if image_key is not None and processed_example[image_key] is not None:
                if len(processed_example[image_key]) > 1:
                    LOG.debug(
                        f"Multiple images detected. Strategy type: {type(self).__name__}, "
                        f"supports_multi_images={self.supports_multi_images}"
                    )
