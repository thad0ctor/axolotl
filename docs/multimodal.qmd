...
`pretraining_dataset`, you do not need to manually calculate `max_steps`. You
can run `axolotl preprocess` on this mode; the prepared Arrow dataset keeps
`images` as image references and `_mm_text` as raw text for the collator.
