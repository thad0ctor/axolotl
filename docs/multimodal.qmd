...
streaming: true
sequence_len: 2048
sample_packing: false                  # optional — set true for packing (see below)
...
- `sample_packing: true` together with a streaming `pretraining_dataset` **and**
  `dataset_prepared_path` — online lookahead packing can't also build a prepared
  cache from the streaming path. Drop `dataset_prepared_path` (or set
  `multimodal_sample_packing_cache_path` for the SSD metadata cache), or use the
  non-streaming `datasets:` route. Packing itself is supported either way.
