...
Per-row validation: each row's `text.count(placeholder) == len(images)` is
enforced at encode time. This is a critical guardrail — LLaVA and Qwen-VL
processors silently accept rows without placeholders and drop the image,
which looks like successful training but teaches nothing.
