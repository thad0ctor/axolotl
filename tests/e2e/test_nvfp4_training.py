...
class TestNVFP4Training:
    ...
    def test_quantize_lm_head_guard_tied_embeddings(self):
        ...
        out = swap._unexclude_lm_head(
            no_cce, TinyModel(tie=True, frozen=True), ["lm_head", "embed_tokens"]
        )
        ...
        with pytest.raises(RuntimeError, match="FROZEN"):
            swap._unexclude_lm_head(
                no_cce, TinyModel(tie=True, frozen=False), ["lm_head", "embed_tokens"]
            )

        # untied: lm_head removed from exclusion, embed_tokens retained
        out = swap._unexclude_lm_head(
            no_cce, TinyModel(tie=False), ["lm_head", "embed_tokens"]
        )
        ...
        with pytest.raises(RuntimeError, match="cut_cross_entropy"):
            swap._unexclude_lm_head(
                cce, TinyModel(tie=False), ["lm_head", "embed_tokens"]
            )

        # untied + cut_cross_entropy + FP4-aware fused CE (cross_entropy.mode=fp4):
        # the guard is relaxed for THIS path (the fused CE reads the packed lm_head
        # directly), so it no longer raises and lm_head leaves the exclusion.
        cce_fused = DictDefault(
            {
                "cut_cross_entropy": True,
                "nvfp4_training": NVFP4Args(
                    enabled=True,
                    quantize=["lm_head"],
        ...
        out = swap._unexclude_lm_head(
            cce_fused, TinyModel(tie=False), ["lm_head", "embed_tokens"]
        )
        assert out == ["embed_tokens"]
