...
4. **Run.**
   ...
   Exit **`0`** clean (also when a gate self-skips as non-applicable, e.g. G6/G7 with no GPU → SKIPPED) · **`1`** findings · **`2`** can't-run-here / could-not-run (kernel won't compile, model unloadable). A no-GPU run of G6/G7 is a SKIPPED (exit 0), not a could-not-run (exit 2). A mixed import tree only warns (non-blocking). `--on-unavailable skip` makes can't-run non-blocking (offer as a checklist choice). GPU gates retry a transient Triton/CUDA compile error and mark a non-compiling kernel could-not-run, not a false finding.
...
- Both static and run gates use the `--repo-root` checkout: `verify_model.main()` prepends `<repo_root>/src` to `sys.path` and `PYTHONPATH`, so run gates `import axolotl` from that source (the installed tree is only a fallback). The warning at startup fires if `import axolotl` still resolves outside `--repo-root` (mixed trees). The harness reports; it never edits `src/`.
