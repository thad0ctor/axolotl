...
| **G2** integration | CPU | adaptive "wired-everywhere" checklist; flags **silent generic fallbacks**, not just missing hooks; also flags a **misplaced feature-incompat guard** (a model-type-gated `raise` in central `utils/config` that belongs in the owning plugin). |
...
1. **Discover (PR mode).** If the user names a PR, run `--from-pr N` (or `--from-diff FILE`): a torch-free pass printing the new `model_config_type`(s), their `base_model`, new-vs-extends, and warnings for registration that static greps miss (CCE pin-bump, native-liger table, processor-class MM routing, transformers bump). Confirm the model, then continue with `--base-model` + `--repo-root <PR checkout>`.
...
4. **Run.**
   ...
   ```
   Prints the report to stdout and returns an exit code (`--report`/`--manifest` also write files; `--quiet` = summary only). Invoke the script directly (the `__main__` guard datasets' multiprocess save needs), not via `python -c`.
   Exit **`0`** clean — incl. a non-applicable self-skip, e.g. G6/G7 with no GPU → SKIPPED · **`1`** findings · **`2`** could-not-run (kernel won't compile, model unloadable). A mixed import tree only warns; `--on-unavailable skip` makes could-not-run non-blocking (offer as a checklist choice). GPU gates retry a transient Triton/CUDA compile error and mark a non-compiling kernel could-not-run, not a false finding.
5. **Interpret.** Call out: **G2 generic-fallback ⚠️** (runs a generic path that may be wrong — review or add a dedicated branch) and **missing ❌** (wire it: multipack / patch_manager / MoE block / …); **G2 feature-incompat guard placement ⚠️** (a model-type-gated `raise` in `utils/config` — relocate to the owning plugin: LigerPlugin / CutCrossEntropyPlugin / PatchManager; G1 still catches the reject either way, so this is a hygiene lead, not a correctness gap); **G1 warned-no-op** (flag accepted but inert); **G4 finding** (prompt trained, assistant masked, or turn terminator masked / #3754 — a chat-template/masking bug); **G7 SHADOW SUSPECT** (kernel changed the math — highest value). On a green run, offer `--emit-test`.
   **Frame the summary as a starting point, not a verdict.** Tell the user this is an automated heuristic smoke gate that surfaces *leads*, not conclusions: every finding *and* every PASS needs independent verification against the model's modeling code, the PR diff, and upstream docs. Don't say a model "is verified / correct"; say the gate "found no flagged issues in what it checks." Name the blind spots — per-arch numerical/attention correctness, transformers-version-coupled breakage, VRAM regressions, subtle tokenization offsets, distributed-only failures. A green run ≠ a guarantee.

## Notes

- `manifest.json` captures seed, resolved flags, package versions, git sha, and GPU for reproducibility.
- Both static and run gates use the `--repo-root` checkout: `main()` prepends `<repo_root>/src` to `sys.path`+`PYTHONPATH` so `import axolotl` resolves there (installed tree is a fallback; a startup warning fires if it still resolves elsewhere — mixed trees). The harness reports; it never edits `src/`.
