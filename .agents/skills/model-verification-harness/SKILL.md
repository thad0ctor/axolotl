...
4. **Run.**
   ...
   Exit **`0`** clean (also when a gate self-skips as non-applicable, e.g. G6/G7 with no GPU → SKIPPED) · **`1`** findings · **`2`** can't-run-here / could-not-run (kernel won't compile, model unloadable, mixed import trees). A no-GPU run of G6/G7 is a SKIPPED (exit 0), not a could-not-run (exit 2). `--on-unavailable skip` makes can't-run non-blocking (offer as a checklist choice). GPU gates retry a transient Triton/CUDA compile error and mark a non-compiling kernel could-not-run, not a false finding.
